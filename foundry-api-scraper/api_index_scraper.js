...
class APIIndexScraper {
  ...
  async scrape() {
    console.log('Starting Foundry VTT API Index scraper...');
      ...
      const endpoints = await page.evaluate(() => {
        ...
        selectors.forEach(selector => {
          ...
          links.forEach(link => {
            ...
            if (href && text && !allLinks.some(l => l.href === href)) {
              allLinks.push({
                href: href,
                text: text,
              });
        ...
        allLinks.forEach(link => {
          ...
          if (url.includes('/classes/') || text.includes('Class')) {
          ...
          } else if (url.includes('/functions/') || text.includes('Function')) {
          ...
          } else if (url.includes('/types/') || text.includes('Type')) {
            results.types.push({
              name: text,
              url: `https://foundryvtt.com/api/${url}`,
              path: url,
          ...
          } else if (url.includes('/modules/') || text.includes('Module')) {
...
// Run the scraper
const scraper = new APIIndexScraper();
