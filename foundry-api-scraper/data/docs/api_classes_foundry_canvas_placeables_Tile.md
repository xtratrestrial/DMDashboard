# Tile | Foundry Virtual Tabletop - API Documentation - Version 13

Source: https://foundryvtt.com/api/classes/foundry.canvas.placeables.Tile.html
...
- new Tile(document: CanvasDocument): canvas.placeables.TileParametersdocument: CanvasDocumentThe Document instance represented by this object
Returns canvas.placeables.TileInherited from PlaceableObject.constructor
...
- refreshShape: {    deprecated: { alias: boolean; since: number; until: number };    propagate: string[];}Deprecatedsince v12
...
##### refreshShape: {    deprecated: { alias: boolean; since: number; until: number };    propagate: string[];}
...
- get aspectRatio(): numberGet the native aspect ratio of the base texture for the Tile sprite
Returns number
...
- get isVisible(): booleanIs this Tile currently visible on the Canvas?
Returns boolean
...
- get occluded(): booleanIs this tile occluded?
Returns boolean
...
- get playing(): booleanIs the tile video playing?
Returns boolean
...
- get volume(): numberThe effective volume at which this Tile should be playing, including the global ambient volume modifier
Returns number
...
- _draw(options?: {}): Promise<void>Parametersoptions: {} = {}Returns Promise<void>Overrides PlaceableObject._draw
- options: {} = {}
...
- _onDragLeftDrop(event: any): false | voidCallback actions which occur on a mouse-move operation.
Parametersevent: anyThe triggering canvas interaction event
Returns false | voidOverrides PlaceableObject._onDragLeftDrop
- event: anyThe triggering canvas interaction event
...
- clear(): canvas.placeables.TileClear the display of the existing object.
Returns canvas.placeables.TileThe cleared object
...
`Protected`- _onHandleDragCancel(event: FederatedEvent<UIEvent | PixiTouch>): voidProtectedHandle cancellation of a drag event for one of the resizing handles
...
Parametersevent: FederatedEvent<UIEvent | PixiTouch>The mousemove event
Returns void
- event: FederatedEvent<UIEvent | PixiTouch>The mousemove event
...
`Protected`- _onHandleDragStart(event: FederatedEvent<UIEvent | PixiTouch>): voidProtectedHandle the beginning of a drag event on a resize handle.
Parametersevent: FederatedEvent<UIEvent | PixiTouch>The mousedown event
Returns void
- event: FederatedEvent<UIEvent | PixiTouch>The mousedown event

`Protected`Handle the beginning of a drag event on a resize handle.
...
`Protected`- _refreshFrame(): voidProtectedRefresh the border frame that encloses the Tile.
Returns void

`Protected`Refresh the border frame that encloses the Tile.
...
`Protected`- _refreshMesh(): voidProtectedRefresh the appearance of the tile.
Returns void

`Protected`Refresh the appearance of the tile.
...
`Protected`- _refreshRotation(): anyProtectedRefresh the rotation.
Returns any
...
`Protected`- _refreshSize(): undefined | GraphicsProtectedRefresh the size.
...
`Protected`- _refreshState(): voidProtectedRefresh the displayed state of the Tile.
Updated when the tile interaction state changes, when it is hidden, or when its elevation changes.
Returns void

`Protected`Refresh the displayed state of the Tile.
Updated when the tile interaction state changes, when it is hidden, or when its elevation changes.
...
### Protected_refreshVideo

`Protected`- _refreshVideo(): voidProtectedRefresh changes to the video playback state.
Returns void

`Protected`Refresh changes to the video playback state.
...
`Static`- createPreview(data: object): PlaceableObjectCreate a preview tile with a background texture instead of an image
Parametersdata: objectInitial data with which to create the preview Tile
Returns PlaceableObject
- data: objectInitial data with which to create the preview Tile
