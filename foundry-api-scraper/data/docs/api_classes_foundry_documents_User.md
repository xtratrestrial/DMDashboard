...
- foundry.documents.collections.Users: The world-level collection of User documents
- foundry.applications.sheets.UserConfig: The User configuration application
...
- new User(    data?: Partial<UserData>,    options?: DocumentConstructionContext,): documents.UserParametersOptionaldata: Partial<UserData> = {}Initial data used to construct the data object. The provided object will be
...
Returns documents.UserInherited from BaseUser.constructor
- Optionaldata: Partial<UserData> = {}Initial data used to construct the data object. The provided object will be
owned by the constructed model instance and may be mutated.
- Optionaloptions: DocumentConstructionContext = {}Context and data validation options which affects initial model construction.
...
Inherited from BaseUser.constructor
...
Inherited from BaseUser._source
...
### ReadonlymovingTokens

`Readonly`Track the Token documents that this User is currently moving.
...
Inherited from BaseUser.parent
...
Inherited from BaseUser._schema


### StaticLOCALIZATION_PREFIXES

`Static`Inherited from BaseUser.LOCALIZATION_PREFIXES
...
Inherited from BaseUser.metadata
...
Returns anyInherited from ClientDocumentMixin(BaseUser).compendium
...
Returns null | stringInherited from ClientDocumentMixin(BaseUser).id
...
Returns booleanInherited from ClientDocumentMixin(BaseUser).inCompendium
...
Returns booleanInherited from ClientDocumentMixin(BaseUser).invalid
...
- get isActiveGM(): booleanIs this User the active GM?
Returns boolean
...
- get isBanned(): booleanA convenience test for whether this User has the NONE role.
Returns booleanInherited from ClientDocumentMixin(BaseUser).isBanned
...
Returns booleanInherited from ClientDocumentMixin(BaseUser).isEmbedded
...
- get isGM(): booleanTest whether the User has a GAMEMASTER or ASSISTANT role in this World?
Returns booleanInherited from ClientDocumentMixin(BaseUser).isGM
...
- get isSelf(): booleanA flag for whether this User is the connected client
Returns boolean
...
- get isTrusted(): booleanA flag for whether the current User is a Trusted Player
Returns boolean
...
- get lastActivityTime(): numberThe timestamp of the last observed activity for the user.
Returns number
...
- get roleLabel(): stringA localized label for this User's role.
Returns string
...
Returns SchemaFieldInherited from ClientDocumentMixin(BaseUser).schema
...
Returns stringInherited from ClientDocumentMixin(BaseUser).uuid
...
Returns {    fields: null | DataModelValidationFailure;    joint: null | DataModelValidationFailure;}Inherited from ClientDocumentMixin(BaseUser).validationFailures
...
Returns typeof DocumentInherited from ClientDocumentMixin(BaseUser).baseDocument
...
Returns stringInherited from ClientDocumentMixin(BaseUser).collectionName
...
Returns abstract.DatabaseBackendInherited from ClientDocumentMixin(BaseUser).database
...
Returns stringInherited from ClientDocumentMixin(BaseUser).documentName
...
Returns booleanInherited from ClientDocumentMixin(BaseUser).hasTypeData
...
Returns Readonly<Record<string, any>>Inherited from ClientDocumentMixin(BaseUser).hierarchy
...
Returns typeof DocumentInherited from ClientDocumentMixin(BaseUser).implementation
...
Returns string[]Inherited from ClientDocumentMixin(BaseUser).TYPES
...
- _configure(__namedParameters?: { pack?: null; parentCollection?: null }): voidParameters__namedParameters: { pack?: null; parentCollection?: null } = {}Returns voidInherited from BaseUser._configure
...
Inherited from BaseUser._configure
...
Returns null | stringInherited from BaseUser._getParentCollection
...
Inherited from BaseUser._getParentCollection
...
Returns voidOverrides BaseUser._onDelete
...
Overrides BaseUser._onDelete


### _onUpdate

- _onUpdate(changed: any, options: any, userId: any): anyPost-process an update operation for a single Document instance. Post-operation events occur for all connected
clients.
...
userId: anyThe id of the User requesting the document update
Returns anyOverrides BaseUser._onUpdate
...
Overrides BaseUser._onUpdate


### _onUpdateTokenTargets

- _onUpdateTokenTargets(targetIds?: string[]): voidInternalUpdate the set of Token targets for the user given an array of provided Token ids.
This function handles changes made elsewhere and does not broadcast to other connected clients.
ParameterstargetIds: string[] = []An array of Token ids which represents the new target set
Returns void
- targetIds: string[] = []An array of Token ids which represents the new target set

`Internal`Update the set of Token targets for the user given an array of provided Token ids.
This function handles changes made elsewhere and does not broadcast to other connected clients.
...
- assignHotbarMacro(    macro: null | documents.Macro,    slot?: null | string | number,    fromSlot?: number,): Promise<documents.User>Assign a Macro to a numbered hotbar slot between 1 and 50
Parametersmacro: null | documents.MacroThe Macro document to assign
Optionalslot: null | string | numberA specific numbered hotbar slot to fill
OptionalfromSlot: number = {}An optional origin slot from which the Macro is being shifted
Returns Promise<documents.User>A Promise which resolves once the User update is complete
- macro: null | documents.MacroThe Macro document to assign
- Optionalslot: null | string | numberA specific numbered hotbar slot to fill
- OptionalfromSlot: number = {}An optional origin slot from which the Macro is being shifted
...
`Optional`A specific numbered hotbar slot to fill

`Optional`An optional origin slot from which the Macro is being shifted
...
- assignPermission(    permission: string,    allowed: boolean,): Promise<undefined | Document<object, DocumentConstructionContext>>Assign a specific boolean permission to this user.
Modifies the user permissions to grant or restrict access to a feature.
Parameterspermission: stringThe permission name from USER_PERMISSIONS
allowed: booleanWhether to allow or restrict the permission
Returns Promise<undefined | Document<object, DocumentConstructionContext>>
- permission: stringThe permission name from USER_PERMISSIONS
- allowed: booleanWhether to allow or restrict the permission
...
- broadcastActivity(    activityData?: ActivityData,    options?: { volatile?: boolean },): voidSubmit User activity data to the server for broadcast to other players.
...
ParametersactivityData: ActivityData = {}An object of User activity data to submit to the server for broadcast.
Optionaloptions: { volatile?: boolean } = {}Optionalvolatile?: booleanIf undefined, volatile is inferred from the activity data.
Returns void
- activityData: ActivityData = {}An object of User activity data to submit to the server for broadcast.
- Optionaloptions: { volatile?: boolean } = {}Optionalvolatile?: booleanIf undefined, volatile is inferred from the activity data.
- Optionalvolatile?: booleanIf undefined, volatile is inferred from the activity data.
...
`Optional`- Optionalvolatile?: booleanIf undefined, volatile is inferred from the activity data.


##### Optionalvolatile?: boolean

`Optional`If undefined, volatile is inferred from the activity data.
...
- can(action: string): booleanTest whether the User is able to perform a certain permission action.
The provided permission string may pertain to an explicit permission setting or a named user role.
Parametersaction: stringThe action to test
Returns booleanDoes the user have the ability to perform this action?
Inherited from BaseUser.can
- action: stringThe action to test
...
Inherited from BaseUser.canUserModify
...
Inherited from BaseUser.createEmbeddedDocuments
...
Inherited from BaseUser.delete
...
SeeDocument.deleteDocuments
Inherited from BaseUser.deleteEmbeddedDocuments
- embeddedName: stringThe name of the embedded Document type
- ids: string[]An array of string ids for each Document to be deleted
- Optionaloperation: DatabaseDeleteOperation = {}Parameters of the database deletion workflow
...
Inherited from BaseUser.getEmbeddedCollection
- embeddedName: stringThe name of the embedded Document type
...
found.
Inherited from BaseUser.getEmbeddedDocument
- embeddedName: stringThe name of the embedded Document type
- id: stringThe id of the child document to retrieve
- Optionaloptions: { invalid?: boolean; strict?: boolean } = {}Additional options which modify how embedded documents are retrieved
Optionalinvalid?: booleanAllow retrieving an invalid Embedded Document.
...
Inherited from BaseUser.getFlag
...
- getHotbarMacros(    page?: number,): { macro: null | documents.Macro; slot: number }[]Get an Array of Macro Documents on this User's Hotbar by page
Parameterspage: number = 1The hotbar page number
Returns { macro: null | documents.Macro; slot: number }[]
- page: number = 1The hotbar page number
...
- getUserLevel(user: any): 0 | 3Get the explicit permission level that a User has over this Document, a value in CONST.DOCUMENT_OWNERSHIP_LEVELS.
...
Returns 0 | 3A numeric permission level from CONST.DOCUMENT_OWNERSHIP_LEVELS
Inherited from BaseUser.getUserLevel
...
- hasPermission(permission: string): booleanTest whether the User has at least a specific permission
Parameterspermission: stringThe permission name from USER_PERMISSIONS to test
Returns booleanDoes the user have at least this permission
Inherited from BaseUser.hasPermission
- permission: stringThe permission name from USER_PERMISSIONS to test
...
- hasRole(role: string | number, exact?: boolean): booleanTest whether the User has at least the permission level of a certain role
Parametersrole: string | numberThe role name from USER_ROLES to test
Optionalexact: boolean = {}Require the role match to be exact
Returns booleanDoes the user have at this role level (or greater)?
Inherited from BaseUser.hasRole
- role: string | numberThe role name from USER_ROLES to test
- Optionalexact: boolean = {}Require the role match to be exact
...
`Optional`Require the role match to be exact
...
- isDesignated(condition: (user: documents.User) => boolean): booleanIs this User the designated User among the Users that satisfy the given condition?
This function calls foundry.documents.collections.Users#getDesignatedUser and compares the designated User
to this User.
Parameterscondition: (user: documents.User) => booleanThe condition the Users must satisfy
Returns booleanIs designated User?
Example// Is the current User the designated User to create Tokens?const isDesignated = game.user.isDesignated(user => user.active && user.can("TOKEN_CREATE"));
Copy
- condition: (user: documents.User) => booleanThe condition the Users must satisfy
...
```javascript
// Is the current User the designated User to create Tokens?const isDesignated = game.user.isDesignated(user => user.active && user.can("TOKEN_CREATE"));
Copy
```
...
Inherited from BaseUser.migrateSystemData
...
- query(    queryName: string,    queryData: object,    queryOptions?: { timeout?: number },): Promise<any>Query this User.
ParametersqueryName: stringThe query name (must be registered in CONFIG.queries)
queryData: objectThe query data (must be JSON-serializable)
OptionalqueryOptions: { timeout?: number } = {}The query options
Optionaltimeout?: numberThe timeout in milliseconds
Returns Promise<any>The query result
- queryName: stringThe query name (must be registered in CONFIG.queries)
- queryData: objectThe query data (must be JSON-serializable)
- OptionalqueryOptions: { timeout?: number } = {}The query options
Optionaltimeout?: numberThe timeout in milliseconds
- Optionaltimeout?: numberThe timeout in milliseconds
...
`CONFIG.queries`The query data (must be JSON-serializable)
...
Returns voidInherited from BaseUser.reset
...
Inherited from BaseUser.setFlag
- scope: stringThe flag scope which namespaces the key
- key: stringThe flag key
- value: anyThe flag value
...
Inherited from BaseUser.testUserPermission
...
Inherited from BaseUser.toJSON
...
Inherited from BaseUser.toObject
...
Returns Generator<any, void, any>YieldsInherited from BaseUser.traverseEmbeddedDocuments
...
Inherited from BaseUser.traverseEmbeddedDocuments
...
Inherited from BaseUser.unsetFlag
- scope: stringThe flag scope which namespaces the key
- key: stringThe flag key
...
Inherited from BaseUser.update
...
SeeDocument.updateDocuments
Inherited from BaseUser.updateEmbeddedDocuments
- embeddedName: stringThe name of the embedded Document type
- updates: object[] = []An array of differential data objects, each used to update a
single Document
- Optionaloperation: DatabaseUpdateOperation = {}Parameters of the database update workflow
...
Inherited from BaseUser.updateSource
...
Inherited from BaseUser.validate
...
Returns voidInherited from BaseUser._initialize
...
Inherited from BaseUser._initialize
...
Inherited from BaseUser._initializeSource
...
`data`Inherited from BaseUser._initializeSource
...
Returns voidInherited from BaseUser._onCreate
...
Inherited from BaseUser._onCreate
...
Inherited from BaseUser._preCreate
- data: objectThe initial data object provided to the document creation request
- options: objectAdditional options which modify the creation request
- user: BaseUserThe User requesting the document creation
...
Inherited from BaseUser._preDelete
...
Inherited from BaseUser._preUpdate
...
Inherited from BaseUser._addDataFieldMigration
...
Returns voidInherited from BaseUser._addDataFieldShim
...
Inherited from BaseUser._addDataFieldShim
...
Optionalvalue?: anyThe value of the shim
Optionalwarning?: stringThe deprecation message
Returns voidInherited from BaseUser._addDataFieldShims
- data: objectThe data object being shimmed
- shims: { [oldKey: string]: string }The mapping of old keys to new keys
- Optionaloptions: { value?: any; warning?: string }Options passed to foundry.utils.logCompatibilityWarning
Optionalvalue?: anyThe value of the shim
...
Inherited from BaseUser._addDataFieldShims
...
Returns voidInherited from BaseUser._clearFieldsRecusively
...
Inherited from BaseUser._clearFieldsRecusively


### Static_initializationOrder

`Static`- _initializationOrder(): Generator<any[], void, unknown>Returns Generator<any[], void, unknown>Inherited from BaseUser._initializationOrder


#### Returns Generator<any[], void, unknown>

Inherited from BaseUser._initializationOrder
...
Returns voidInherited from BaseUser._logDataFieldMigration
- oldKey: stringThe old field name
- newKey: stringThe new field name
- Optionaloptions: object = {}Options passed to foundry.utils.logCompatibilityWarning
...
Inherited from BaseUser._logDataFieldMigration
...
Inherited from BaseUser.canUserCreate
- user: BaseUserThe User being tested
...
Inherited from BaseUser.cleanData
...
`source`Inherited from BaseUser.cleanData
...
Inherited from BaseUser.create
...
`const data = [{name: "Special Sword", type: "weapon"}];const created = await Item.implementation.create(data, {pack: "mymodule.mypack"});`Inherited from BaseUser.create
...
Inherited from BaseUser.createDocuments
...
`const data = [{name: "Compendium Actor", type: "character", img: "path/to/profile.jpg"}];const created = await Actor.implementation.createDocuments(data, {pack: "mymodule.mypack"});`Inherited from BaseUser.createDocuments


### StaticdefineSchema

`Static`- defineSchema(): {    _id: DocumentIdField;    _stats: DocumentStatsField;    avatar: FilePathField;    character: ForeignDocumentField;    color: ColorField;    flags: DocumentFlagsField;    hotbar: ObjectField;    name: StringField;    password: StringField;    passwordSalt: StringField;    permissions: ObjectField;    pronouns: StringField;    role: NumberField;}Define the data schema for documents of this type.
The schema is populated the first time it is accessed and cached for future reuse.
Returns {    _id: DocumentIdField;    _stats: DocumentStatsField;    avatar: FilePathField;    character: ForeignDocumentField;    color: ColorField;    flags: DocumentFlagsField;    hotbar: ObjectField;    name: StringField;    password: StringField;    passwordSalt: StringField;    permissions: ObjectField;    pronouns: StringField;    role: NumberField;}Inherited from BaseUser.defineSchema

Define the data schema for documents of this type.
The schema is populated the first time it is accessed and cached for future reuse.


#### Returns {    _id: DocumentIdField;    _stats: DocumentStatsField;    avatar: FilePathField;    character: ForeignDocumentField;    color: ColorField;    flags: DocumentFlagsField;    hotbar: ObjectField;    name: StringField;    password: StringField;    passwordSalt: StringField;    permissions: ObjectField;    pronouns: StringField;    role: NumberField;}

Inherited from BaseUser.defineSchema
...
Inherited from BaseUser.deleteDocuments
...
`const actor = await pack.getDocument(documentId);const deleted = await Actor.implementation.deleteDocuments([actor.id], {pack: "mymodule.mypack"});`Inherited from BaseUser.deleteDocuments
...
Inherited from BaseUser.fromJSON
...
Returns DataModel<object, DataModelConstructionContext>Inherited from BaseUser.fromSource
...
Inherited from BaseUser.fromSource
...
Inherited from BaseUser.getCollectionName
...
`Actor.implementation.getCollectionName("Item");// returns "items"`Inherited from BaseUser.getCollectionName
...
Inherited from BaseUser.migrateData
...
`source`Inherited from BaseUser.migrateData
...
Parameterssource: objectThe candidate source data from which the model will be constructed
Returns objectMigrated source data, which is the same object as the source argument
Inherited from BaseUser.migrateDataSafe
- source: objectThe candidate source data from which the model will be constructed
...
`source`Inherited from BaseUser.migrateDataSafe
...
the data argument
Inherited from BaseUser.shimData
...
`data`Inherited from BaseUser.shimData
...
Inherited from BaseUser.updateDocuments
...
`const actor = await pack.getDocument(documentId);const updated = await Actor.implementation.updateDocuments([{_id: actor.id, name: "New Name"}],  {pack: "mymodule.mypack"});`Inherited from BaseUser.updateDocuments
...
Inherited from BaseUser.validateJoint
...
Returns Promise<void>Inherited from BaseUser._onCreateOperation
...
Inherited from BaseUser._onCreateOperation
...
Returns Promise<void>Inherited from BaseUser._onDeleteOperation
...
Inherited from BaseUser._onDeleteOperation
...
Returns Promise<void>Inherited from BaseUser._onUpdateOperation
...
Inherited from BaseUser._onUpdateOperation
...
Inherited from BaseUser._preCreateOperation
...
Inherited from BaseUser._preDeleteOperation
...
Inherited from BaseUser._preUpdateOperation
