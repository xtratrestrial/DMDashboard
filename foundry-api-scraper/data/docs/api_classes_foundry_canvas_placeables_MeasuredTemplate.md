...
Source: https://foundryvtt.com/api/classes/foundry.canvas.placeables.MeasuredTemplate.html
...
- PlaceableObjectMeasuredTemplate
- MeasuredTemplate
...
- new MeasuredTemplate(    document: CanvasDocument,): canvas.placeables.MeasuredTemplateParametersdocument: CanvasDocumentThe Document instance represented by this object
Returns canvas.placeables.MeasuredTemplateInherited from PlaceableObject.constructor
...
#### Returns canvas.placeables.MeasuredTemplate
...
### Protected_borderThickness

`Protected`Internal property used to configure the control border thickness
...
- get highlightId(): stringA unique identifier which is used to uniquely identify related objects like a template effect or grid highlight.
Returns string
...
- get isAuthor(): booleanA convenient reference for whether the current User is the author of the MeasuredTemplate document.
Returns boolean
...
- get isVisible(): booleanIs this MeasuredTemplate currently visible on the Canvas?
Returns boolean
...
- _canHUD(user: any, event: any): booleanCan the User access the HUD for this Placeable Object?
Parametersuser: anyThe User performing the action. Always equal to game.user.
event: anyThe pointer event if this function was called by
foundry.canvas.interaction.MouseInteractionManager.
Returns booleanOverrides PlaceableObject._canHUD
- user: anyThe User performing the action. Always equal to game.user.
- event: anyThe pointer event if this function was called by
foundry.canvas.interaction.MouseInteractionManager.
...
- _canView(user: any, event: any): anyParametersuser: anyevent: anyReturns anyOverrides PlaceableObject._canView
- user: any
- event: any
...
- _getTargetAlpha(): 0.8 | 1Returns 0.8 | 1Overrides PlaceableObject._getTargetAlpha


#### Returns 0.8 | 1

Overrides PlaceableObject._getTargetAlpha
...
- clear(): canvas.placeables.MeasuredTemplateClear the display of the existing object.
Returns canvas.placeables.MeasuredTemplateThe cleared object
...
- highlightGrid(): voidHighlight the grid squares which should be shown under the area of effect
Returns void
...
- rotate(angle: any, snap: any): Promise<canvas.placeables.MeasuredTemplate>Parametersangle: anysnap: anyReturns Promise<canvas.placeables.MeasuredTemplate>Overrides PlaceableObject.rotate
- angle: any
- snap: any
...
#### Returns Promise<canvas.placeables.MeasuredTemplate>

Overrides PlaceableObject.rotate


### testPoint

- testPoint(point: Point): booleanIs the given point contained in the template's shape?
Parameterspoint: PointThe point
Returns booleanIs contained?
- point: PointThe point
...
### Protected_computeShape

`Protected`- _computeShape(): Rectangle | Polygon | CircleProtectedCompute the geometry for the template using its document data.
...
`Protected`Compute the geometry for the template using its document data.
Subclasses can override this method to take control over how different shapes are rendered.
...
### Protected_getGridHighlightPositions

`Protected`- _getGridHighlightPositions(): Point[]ProtectedGet an array of points which define top-left grid spaces to highlight for square or hexagonal grids.
Returns Point[]

`Protected`Get an array of points which define top-left grid spaces to highlight for square or hexagonal grids.


#### Returns Point[]


### Protected_getGridHighlightShape

`Protected`- _getGridHighlightShape(): Rectangle | Polygon | CircleProtectedGet the shape to highlight on a Scene which uses grid-less mode.
Returns Rectangle | Polygon | Circle

`Protected`Get the shape to highlight on a Scene which uses grid-less mode.
...
`Protected`- _refreshPosition(): voidProtectedRefresh the position of the MeasuredTemplate
Returns void

`Protected`Refresh the position of the MeasuredTemplate


#### Returns void


### Protected_refreshRulerText

`Protected`- _refreshRulerText(): voidProtectedUpdate the displayed ruler tooltip text
Returns void

`Protected`Update the displayed ruler tooltip text
...
`Protected`- _refreshShape(): voidProtectedRefresh the underlying geometric shape of the MeasuredTemplate.
Returns void

`Protected`Refresh the underlying geometric shape of the MeasuredTemplate.
...
`Protected`- _refreshState(): voidProtectedRefresh the displayed state of the MeasuredTemplate.
This refresh occurs when the user interaction state changes.
Returns void

`Protected`Refresh the displayed state of the MeasuredTemplate.
This refresh occurs when the user interaction state changes.
...
### Protected_refreshTemplate

`Protected`- _refreshTemplate(): voidProtectedRefresh the display of the template outline and shape.
Subclasses may override this method to take control over how the template is visually rendered.
Returns void

`Protected`Refresh the display of the template outline and shape.
Subclasses may override this method to take control over how the template is visually rendered.
...
### StaticgetCircleShape

`Static`- getCircleShape(distance: number): Polygon | CircleGet a Circular area of effect given a radius of effect
Parametersdistance: numberThe radius of the circle in grid units
Returns Polygon | Circle
- distance: numberThe radius of the circle in grid units
...
`Static`- getConeShape(    distance: number,    direction: number,    angle: number,): Polygon | CircleGet a Conical area of effect given a direction, angle, and distance
Parametersdistance: numberThe radius of the cone in grid units
direction: numberThe direction of the cone in degrees
angle: numberThe angle of the cone in degrees
Returns Polygon | Circle
- distance: numberThe radius of the cone in grid units
- direction: numberThe direction of the cone in degrees
- angle: numberThe angle of the cone in degrees
...
`Static`- getRayShape(distance: number, direction: number, width: number): PolygonGet a rotated Rectangular area of effect given a width, height, and direction
Parametersdistance: numberThe length of the ray in grid units
direction: numberThe direction of the ray in degrees
width: numberThe width of the ray in grid units
Returns Polygon
- distance: numberThe length of the ray in grid units
- direction: numberThe direction of the ray in degrees
- width: numberThe width of the ray in grid units
...
`Static`- getRectShape(distance: number, direction: number): RectangleGet a Rectangular area of effect given a width and height
Parametersdistance: numberThe length of the diagonal in grid units
direction: numberThe direction of the diagonal in degrees
Returns Rectangle
- distance: numberThe length of the diagonal in grid units
- direction: numberThe direction of the diagonal in degrees
