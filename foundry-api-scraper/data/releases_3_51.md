# Release 0.3.6 | Foundry Virtual Tabletop

Source: https://foundryvtt.com/releases/3.51
...
`#``/roll 4d6kh # Rolling for ability scores!``/w``/whisper`
### Core Bug Fixes
...
`img``unregisterSheet``CONFIG.Token`
...
`ChatMessage``type``CHAT_MESSAGE_TYPES``--world``confirmDialog()``options.height = "auto"``Number.isNumeric()`
